...
public class StorageVersionMigrationTest
{
    ...
    public void migrateLegacyToV2( final TestContext context )
    {
        ...
        final Registry<Enchantment> enchantmentRegistry = player
            .getRegistryManager()
        ...
        context.assertEquals(
            ((HungerManagerAccessor) player.getHungerManager()).getExhaustion(),
            0.2372941f,
            "exhaustion"
        );
        ...
        if ( MinecraftVersion.CURRENT.getSaveVersion().getId() <= 3819 )
        {
            ...
            context.assertEquals( chest.getCount(), 1, "%s count".formatted( Items.ELYTRA ) );
            context.assertEquals( chest.getCount(), 1, "Elytra count" );
            context.assertEquals( chest.getDamage(), 6, "Elytra damage" );
            context.assertEquals(
                EnchantmentHelper
                    .getEnchantments( chest )
                    .getLevel( enchantmentRegistry.getOrThrow( Enchantments.UNBREAKING ) ), 3, "Unbreaking level"
            );
            context.assertEquals(
                EnchantmentHelper
                    .getEnchantments( chest )
                    .getLevel( enchantmentRegistry.getOrThrow( Enchantments.MENDING ) ), 1, "Mending level"
            );
            context.assertEquals( chest.getName(), "Can I put a wang on this?", "custom item name" );
        }
