        ...
        if ( MinecraftVersion.CURRENT.getSaveVersion().getId() <= 3819 )
        {
            ...
            context.assertEquals(
                EnchantmentHelper
                    ...
                    .getLevel( enchantmentRegistry.getEntry( Enchantments.UNBREAKING ).orElseThrow() ), 3, "Unbreaking level"
            );
            context.assertEquals(
                EnchantmentHelper
                    .getEnchantments( chest )
                    .getLevel( enchantmentRegistry.getEntry( Enchantments.MENDING ).orElseThrow() ), 1, "Mending level"
            );
